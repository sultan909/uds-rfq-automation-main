...
export async function POST(
  ...
  try {
    ...
    // Calculate requested total prices for items
    const itemsWithTotals = body.responseItems.map(item => ({
      ...item,
    ...
    if (itemsWithTotals.length > 0) {
      const responseItemsData = itemsWithTotals.map(item => ({
        ...
        requestedTotalPrice: item.requestedTotalPrice,
    ...
    // Fetch the complete response with related data
